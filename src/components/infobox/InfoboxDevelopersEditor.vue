...
<template>
    <GenericListForm v-bind="$attrs" v-model="model" :label="$attrs.label as string || 'Developers'" :hideLabel="true"
        ...
        <template #actions="{ item, index, update }">
            ...
            <label class="text-[10px] uppercase font-bold text-surface-400 cursor-pointer"
        ...
        <template #extra="{ item, index, update }">
            <span class="text-[10px] uppercase font-bold text-surface-500">{{ item.type === 'porter' ? 'OS' : 'Subtitle'
                }}</span>
