...
<template>
    <nav
        ...
        <div class="flex items-center sticky top-0 z-10 bg-surface-50/80 dark:bg-surface-950/60 backdrop-blur-sm px-2 py-2.5 border-b border-surface-200/60 dark:border-surface-800/60"
            ...
            <span v-if="!collapsed"
                class="pl-1.5 text-xs font-bold uppercase tracking-wider text-surface-400 dark:text-surface-500 select-none">
                Sections
            </span>
                ...
                <template v-for="item in group.items" :key="item.key">
                    <button type="button"
                        ...
                        :class="[
                            ...
                            !isVisible(panelVisibility, item.key) ? 'opacity-50' : '',
                            activeKey === item.key
