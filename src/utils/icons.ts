...
export const getIconSrc = (val: string) => {
    ...
    // OS and Platform specific icons
    if (v === 'windows') return iconOsWindows;
    if (v === 'windows 3.x') return iconOsWin3x;
    if (v === 'dos') return iconOsDos;
    if (v === 'mac os') return iconOsMacos;
    if (v === 'os x' || v === 'macos' || v === 'ios' || v === 'mac app store') return iconOsMac;
    if (v === 'linux' || v === 'android') return iconOsLinux;
    if (v === 'pc booter') return iconOsBooter;

    // Consoles (fallback support just in case)
    if (v === 'nintendo switch') return iconInputNintendo;
    if (v === 'playstation 4' || v === 'playstation 5') return iconInputDualshock;
    if (v === 'xbox one' || v === 'xbox series x|s') return iconInputXbox;

    // DRM-specific icons
    if (v === 'drm-free') return iconDrmFree;
    if (v === 'account') return iconDrmAccount;
    if (v === 'activation') return iconDrmActivation;
    if (v === 'actlimit') return iconDrmActlimit;
    if (v === 'disc') return iconDrmDisc;
    if (v === 'physical') return iconDrmPhysical;
    if (v === 'floppy') return iconDrmFloppy;
    if (v === 'dongle') return iconDrmDongle;
    if (v === 'key') return iconDrmKey;
    if (v === 'online') return iconDrmOnline;
    if (v === 'unknown') return iconDrmUnknown;

    // DRM Store/Launcher-specific icons
    if (v === 'battle.net') return iconStoreBattlenet;
    if (v === 'bethesda') return iconStoreBethesda;
    if (v === 'ea app') return iconStoreEa;
    if (v === 'epic games launcher') return iconStoreEpicgames;
    if (v === 'gfwl') return iconStoreGfwl;
    if (v === 'gog') return iconStoreGogcom;
    if (v === 'macapp') return iconStoreMas;
    if (v === 'meta store') return iconStoreMeta;
    if (v === 'microsoft store') return iconStoreMicrosoft;
    if (v === 'rockstar games launcher') return iconStoreRockstargames;
    if (v === 'steam') return iconStoreSteam;
    if (v === 'twitch') return iconStoreTwitch;
    if (v === 'ubisoft connect') return iconStoreUbisoft;
